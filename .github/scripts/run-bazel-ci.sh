...
if [[ "${RUNNER_OS:-}" == "Windows" ]]; then
  ...
  if [[ $windows_cross_compile -eq 1 ]]; then
    ...
    for drive in "${workspace_drive,,}" "${system_drive,,}"; do
      ...
      drive_upper="${drive^^}"
      bash_windows_path="${drive_upper}:\\usr\\bin\\bash.exe"
      bash_no_ext_windows_path="${drive_upper}:\\usr\\bin\\bash"
      bash_no_ext_windows_long_path="\\\\?\\${bash_no_ext_windows_path}"
      powershell.exe -NoProfile -Command "[System.IO.File]::Copy('${bash_windows_path}', '${bash_no_ext_windows_long_path}', \$true)"
