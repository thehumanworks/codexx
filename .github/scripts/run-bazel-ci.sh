...
if [[ "${RUNNER_OS:-}" == "Windows" ]]; then
  ...
  if [[ $windows_cross_compile -eq 1 ]]; then
    # Keep genrules runnable on both the Windows runner and Linux RBE workers.
    # Bazel's auto-detected Windows Git Bash path does not exist on RBE.
    post_config_bazel_args+=("--shell_executable=bash")
  fi
