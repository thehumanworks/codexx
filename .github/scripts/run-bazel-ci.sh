...
if [[ "${RUNNER_OS:-}" == "Windows" ]]; then
  ...
  if [[ $windows_cross_compile -eq 1 ]]; then
    ...
    # treats `/usr/bin/bash` as rooted on the system drive. Put the same Bash
    # shim on both drives so local TestRunner actions and Linux RBE actions can
    # share one `--shell_executable` value.
    bash_path="$(command -v bash)"
    bash_dir="$(dirname "$bash_path")"
    bash_runtime_files=("$bash_path")
    for dll in "$bash_dir"/*.dll; do
      if [[ -e "$dll" ]]; then
        bash_runtime_files+=("$dll")
      fi
    done
    ...
    for drive in "${workspace_drive,,}" "${system_drive,,}"; do
      ...
      cp "${bash_runtime_files[@]}" "$portable_shell_dir/"
      "$portable_shell_dir/bash.exe" -lc true
    done
