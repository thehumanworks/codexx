...
fn bundled_bwrap_candidates_for_exe(exe: &Path) -> Vec<PathBuf> {
    ...
    if let Some(path) = bazel_test_bwrap_candidate() {
        candidates.push(path);
    }
    candidates
}

fn bazel_test_bwrap_candidate() -> Option<PathBuf> {
    if option_env!("BAZEL_PACKAGE").is_none() || !bazel_runfiles_env_present() {
        return None;
    }
    ...
    resolve_bazel_runfile(raw.to_str()?)
}

fn bazel_runfiles_env_present() -> bool {
    std::env::var_os("RUNFILES_DIR").is_some()
        || std::env::var_os("TEST_SRCDIR").is_some()
        || std::env::var_os("RUNFILES_MANIFEST_FILE").is_some()
}

fn resolve_bazel_runfile(logical_path: &str) -> Option<PathBuf> {
