...
async fn pre_tool_use_blocks_mcp_tool_before_execution_with_legacy_prefixed_names() -> Result<()> {
    pre_tool_use_blocks_mcp_tool_before_execution(/*prefix_mcp_tool_names*/ true, "mcp__rmcp").await
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn pre_tool_use_blocks_mcp_tool_before_execution_with_non_prefixed_names() -> Result<()> {
    pre_tool_use_blocks_mcp_tool_before_execution(/*prefix_mcp_tool_names*/ false, "rmcp").await
}
...
{
    post_tool_use_records_mcp_tool_payload_and_context(/*prefix_mcp_tool_names*/ false, "rmcp")
        .await
}
