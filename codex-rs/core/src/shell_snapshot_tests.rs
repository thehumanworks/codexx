...
async fn cleanup_stale_snapshots_removes_orphans_and_keeps_live() -> Result<()> {
    ...
    cleanup_stale_snapshots(&codex_home, ThreadId::new(), crate::StateDbAccess::none()).await?;
...
async fn cleanup_stale_snapshots_skips_active_session() -> Result<()> {
    ...
    cleanup_stale_snapshots(&codex_home, active_session, crate::StateDbAccess::none()).await?;
