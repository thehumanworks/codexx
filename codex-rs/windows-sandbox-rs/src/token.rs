...
///
/// The elevated runner needs owner-scoped access for kernel objects such as named pipes created
/// by sandboxed child processes, but adding the sandbox account's user SID back into the
/// restricting set can also re-admit ambient filesystem permissions outside the declared write
/// roots. OWNER RIGHTS preserves the owner-only object access without broadening file writes.
///
/// # Safety
/// Caller must close the returned token handle; base_token must be a valid primary token.
pub unsafe fn create_workspace_write_token_with_caps_and_owner_rights_from(
    ...
    let owner_rights = convert_string_sid_to_sid(OWNER_RIGHTS_SID)
        .ok_or_else(|| anyhow!("invalid owner rights SID"))?;
    let res = create_token_with_caps_from(base_token, psid_capabilities, &[owner_rights]);
    LocalFree(owner_rights as HLOCAL);
    res
}
...
/// Create a restricted token that includes all provided capability SIDs plus OWNER RIGHTS.
///
/// This mirrors the workspace-write helper above so the elevated read-only path retains the same
/// owner-scoped kernel-object behavior without granting the sandbox account ambient filesystem
/// scope.
///
/// # Safety
/// Caller must close the returned token handle; base_token must be a valid primary token.
