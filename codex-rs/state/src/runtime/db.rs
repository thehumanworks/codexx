...
impl InstrumentedDb {
    ...
    pub(super) async fn read<T, F>(&self, operation: DbOperation, future: F) -> anyhow::Result<T>
    where
        F: Future<Output = anyhow::Result<T>>,
    {
        self.record_operation(operation, DbAccess::Read, future)
            .await
    }

    pub(super) async fn write<T, F>(&self, operation: DbOperation, future: F) -> anyhow::Result<T>
    where
        F: Future<Output = anyhow::Result<T>>,
    {
        self.record_operation(operation, DbAccess::Write, future)
            .await
    }

    pub(super) async fn transaction<T, F>(
        &self,
    ...
    where
        F: Future<Output = anyhow::Result<T>>,
    {
        self.record_operation(operation, DbAccess::Transaction, future)
            .await
    }

    pub(super) async fn maintenance<T, F>(
        &self,
        operation: DbOperation,
        future: F,
    ) -> anyhow::Result<T>
    where
        F: Future<Output = anyhow::Result<T>>,
    {
        self.record_operation(operation, DbAccess::Maintenance, future)
            .await
    }
