                ...
                self.pending_reviews.insert(
                    ...
                    PendingReviewState {
                        ...
                        subject_name: if is_network_access_review {
                            ...
                            app_server_guardian_command_tool_name(params.source).to_string()
                        },
                        trigger,
                        started_at_ms,
    ...
    fn ingest_server_response(
        ...
        match response {
            ...
            ServerResponse::PermissionsRequestApproval {
                ...
                let (status, resolution) = if response.permissions.network.is_none()
                    && response.permissions.file_system.is_none()
                {
...
mod tests {
    ...
    #[test]
    fn app_server_guardian_command_tool_name_maps_all_sources() {
        assert_eq!(
            app_server_guardian_command_tool_name(AppServerGuardianCommandSource::Shell),
            "shell"
        );
        assert_eq!(
            app_server_guardian_command_tool_name(AppServerGuardianCommandSource::UnifiedExec),
            "unified_exec"
        );
    }
