...
mod tests {
    ...
    #[test]
    fn selecting_modal_by_exact_match() {
        let mut popup = CommandPopup::new(CommandPopupFlags::default(), Vec::new());
        popup.on_composer_text_change("/modal".to_string());

        match popup.selected_item() {
            Some(CommandItem::Builtin(cmd)) => {
                assert_eq!(cmd, SlashCommand::Modal);
                assert_eq!(
                    cmd.description(),
                    "connect to Modal: /modal [sandbox_id]:[workdir]"
                );
            }
            Some(CommandItem::ServiceTier(command)) => {
                panic!("expected modal command, got service tier {command:?}")
            }
            None => panic!("expected a selected command for exact match"),
        }
    }
