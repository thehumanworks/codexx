...
async fn fast_status_indicator_requires_chatgpt_auth() {
    ...
    assert!(model_supports_fast_mode(&chat, "gpt-5.4"));
    chat.set_service_tier(Some(ServiceTier::from(SERVICE_TIER_PRIORITY)));
...
async fn fast_status_indicator_is_hidden_for_models_without_fast_support() {
    ...
    set_fast_mode_test_catalog(&mut chat);
    assert!(!model_supports_fast_mode(&chat, "gpt-5.3-codex"));
    chat.set_service_tier(Some(ServiceTier::from(SERVICE_TIER_PRIORITY)));
    set_chatgpt_auth(&mut chat);
    set_fast_mode_test_catalog(&mut chat);
    assert!(!model_supports_fast_mode(&chat, "gpt-5.3-codex"));
